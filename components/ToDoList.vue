  ...
  <v-card>
    <v-list>
      <nuxt-link
        ...
        :key="toDo.id"
      >
        <v-list-item :input-value="toDo.done" color="grey">
          <v-btn icon @click.prevent="toggleDone(toDo)">
          ...
          <edit-menu :toDo="toDo"></edit-menu>
          <v-btn icon @click.prevent="deleteToDo(toDo.id)">
...
export default defineComponent({
  components: { EditMenu },
  setup() {
    const { toDos, deleteToDo, toggleDone } = inject(
      ToDoListKey,
      useToDoList()
    ) as ToDoListStore;
